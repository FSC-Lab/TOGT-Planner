...
void CubicSpline::set_boundary(CubicSpline::bd_type left, double left_value,
                               CubicSpline::bd_type right, double right_value,
...
void CubicSpline::set_points(const std::vector<double>& x,
  ...
  if (cubic_spline == true) {  // cubic CubicSpline interpolation
...
void CubicSpline::coeffs(double x, double& x_s,
                        std::vector<double>& poly_coeff) const {
  size_t n = m_x.size();
  // find the closest point m_x[idx] < x, idx=0 even if x<m_x[0]
  std::vector<double>::const_iterator it;
  it = std::lower_bound(m_x.begin(), m_x.end(), x);
  ...
  poly_coeff.clear();
  poly_coeff.resize(4);
  // TODO: assume the index increase mono. However, distance always decrease.
  if (x < m_x[0]) {
    // return first point
    // poly_coeff[0] = 0.0;
    // poly_coeff[1] = m_b0;
    ...
    // return last point
    // poly_coeff[0] = 0.0;
    // poly_coeff[1] = m_b[n-1];
    // poly_coeff[2] = m_c[n-1];
    // poly_coeff[3] = m_y[n-1];
    poly_coeff[0] = 0.0;
    poly_coeff[1] = 0.0;
    poly_coeff[2] = 0.0;
    poly_coeff[3] = m_y[n - 1];
    ...
    poly_coeff[0] = m_a[idx];
    poly_coeff[1] = m_b[idx];
    poly_coeff[2] = m_c[idx];
    poly_coeff[3] = m_y[idx];
  }
}

std::array<double, 5> CubicSpline::coeffs(double x) const {
  size_t n = m_x.size();
  // find the closest point m_x[idx] < x, idx=0 even if x<m_x[0]
  std::vector<double>::const_iterator it;
  it = std::lower_bound(m_x.begin(), m_x.end(), x);
...
bool CubicSpline::sample_points(double x, double& y_s, double& y_e) {
  int n = m_x.size();
  // find the closest point m_x[idx] < x, idx=0 even if x<m_x[0]
  std::vector<double>::const_iterator it;
  it = std::lower_bound(m_x.begin(), m_x.end(), x);
