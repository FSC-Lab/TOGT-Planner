...
bool TrajSE3Data::allocateSpace() {
  ...
  std::cout << "spatialVarDim: " << spatialVarDim << std::endl;
  std::cout << "headingVarDim: " << headingVarDim << std::endl;
  std::cout << "temporalVarDim: " << temporalVarDim << std::endl;
...
void TrajSE3Data::backwardY(const Eigen::VectorXd &Y,
  ...
  for (int i = 0; i < Y.size(); i++) {
    ...
    Z.segment(k, 2) = Eigen::Vector2d(cos(Y(i)), sin(Y(i))); // (x=cos(\theta), y=sin(\theta))
    k += 2;
  }
}
