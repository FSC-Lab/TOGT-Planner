...
bool TrajSE3Solver::solve(const PVAJ3D &initState,
            ...
            const Eigen::Matrix<double, 1, 4> &initYaw,
            const Eigen::Matrix<double, 1, 4> &endYaw,
  ...
  // minco_yaw.setParameters(initYaw, Eigen::Vector2d(1.0, 1.0));
  // auto yaw_coeffs = minco_yaw.getCoeffs();
  // std::cout << "yaw_coeffs: " << yaw_coeffs << std::endl;
...
double TrajSE3Solver::addPenaltyCost(const Eigen::VectorXd &T,
  ...
  for (int i = 0; i < pieceNum; i++) {
    ...
    for (int j = 0; j <= numCheckPerPiece; j++) {
      ...
      std::cout << "totalGradHeading: " << totalGradHeading.transpose() << std::endl;
