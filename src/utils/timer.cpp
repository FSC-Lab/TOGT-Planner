...
Timer::Timer(const std::string name, const std::string module)
  : name_(name),
    ...
    timing_last_(0.0),
    timing_S_(0.0),
    timing_min_(std::numeric_limits<double>::max()),
    timing_max_(0.0),
    n_samples_(0) {}
...
double Timer::toc() {
  ...
  if (timing_mean_ <= 0.0) {
    ...
    const double timing_mean_prev = timing_mean_;
    timing_mean_ =
...
double Timer::operator()() const { return timing_mean_; }

double Timer::mean() const { return timing_mean_; }

double Timer::last() const { return timing_last_; }

double Timer::min() const { return timing_min_; }

double Timer::max() const { return timing_max_; }

double Timer::std() const { return std::sqrt(timing_S_ / n_samples_); }
