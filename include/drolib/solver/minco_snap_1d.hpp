...
class MincoSnap1D {
 ...
 public:
  ...
  void getEnergyPartialGradByCoeffs(Eigen::MatrixX1d &gdC) const {
    gdC.resize(8 * N, 1);
    for (int i = 0; i < N; i++) {
...
void propagateGrad(const Eigen::MatrixX1d &partialGradByCoeffs,
                     const Eigen::VectorXd &partialGradByTimes,
                     Eigen::MatrixX1d &gradByPoints,
                     Eigen::VectorXd &gradByTimes) {
    gradByPoints.resize(1, N - 1);
    gradByTimes.resize(N);
    Eigen::MatrixX1d adjGrad = partialGradByCoeffs;
    A.solveAdj(adjGrad);
    ...
    gradByTimes(N - 1) =
        B2.cwiseProduct(adjGrad.block<4, 1>(8 * N - 4, 0)).sum();
