...
class Yaml {
 public:
  ...
  Yaml(const std::string& keyspace, const std::string& key,
    ...
    //TODO: We don't clean sequences of size 1
    // if (map_.empty()) {
    //   raw_ = seq_[0].raw_;
    //   map_ = seq_[0].map_;
    //   seq_ = seq_[0].seq_;
 ...
 private:
  ...
  template<typename Derived>
  void as_vector(Derived& vector) const {
    if (isNull())
      throw YamlException(getFullKeyName(),
    ...
    try {
      for (size_t i = 0; i < n; ++i)
        vector[i] = seq_[i].as<typename std::string>(); 
    } catch (std::exception& e) {
      throw YamlException(
        getFullKeyName(),
...
// Constraint the typename T to obligatorily be a sub-class of A, 
// otherwise I'll get a compiling error:
